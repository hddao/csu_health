...
# School expense
expense <- openxlsx::read.xlsx("DATA/Raw/CDE-School/CDE_expense.xlsx") %>%
  janitor::clean_names()
...
# Merge with data on school expense
expense_df <- expense %>%
  dplyr::mutate(cdenumber = cdenumber %>% as.character()) %>%
  dplyr::rowwise() %>%
  dplyr::mutate(dplyr::across(learning_environment:operations_maintenance, ~./total_spending,
                              .names = 'pct_{col}')) %>%
  dplyr::mutate(dplyr::across(learning_environment:operations_maintenance, ~./enrollment,
                              .names = 'per_{col}')) %>%
  dplyr::ungroup()
...
save_data(expense_df, "DATA/Processed/Aim1/expense_df", "DATA/Processed/Aim1/Archived/expense_df")
