...
covar <- vars_list %>% 
  ...
  dplyr::filter(!(varname %in% c("ses_medianhhincome", "ses_crowding",
                                 ...
                                 "ses_unemployed", "ses_totalunexcuseddays"))) %>% 
...
covar_multivar <- c("ses_edu_highschoolmore",
...
combos <- tidyr::crossing(v = covar_multivar %>% paste0(collapse = " + "),
                          o = c("elascalescore", "mathscalescore"),
                          g = 3:11)
...
# openxlsx::write.xlsx(multivar_list, "outputs/multivar_list_GEOID_ses_student_school_ieqs.xlsx", 
#                      firstRow = TRUE, 
#                      overwrite = TRUE)
