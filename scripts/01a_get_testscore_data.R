...
# Load data ---------------------------------------------------------------
# r import_testscore
raw.testscore1 <- readr::read_csv("DATA/Raw/Adams12StudentData/Final 2015-2018 data set/Final data file 2015-2018 with geocoding.csv")
raw.testscore2 <- readxl::read_excel("DATA/Raw/Adams12StudentData/2018-19 Data/Final data file 2018-2019 with geocoding.xlsx")
...
data.testscore <- dplyr::bind_rows(
  raw.testscore1 %>% 
...
sum(is.na(data.testscore$ELAScaleScore))
sum(is.na(data.testscore$mathScaleScore))
sum(is.na(data.testscore$ELAScaleScore) | is.na(data.testscore$mathScaleScore))
sum(is.na(data.testscore$ELAScaleScore) & is.na(data.testscore$mathScaleScore))


# # Remove rows with missing values for either ELAScaleScore or mathScaleScore
data.testscore <- data.testscore %>% dplyr::filter(!is.na(ELAScaleScore) | !is.na(mathScaleScore))
...
# r dedup
data.testscore.dedup <-  data.testscore %>% 
...
list.recheckxy <- data.testscore.dedup %>% 
...
data.testscore.aim1 <- data.testscore %>% 
  # Remove duplicated observation at all variables except id_dao
  dplyr::distinct(dplyr::across(-id_dao), .keep_all = TRUE) %>% 
  dplyr::arrange(studentKey) %>% 
...
# Clean variables 
data.testscore.aim1 <- data.testscore.aim1 %>% 
  # Clean var ethnicity 
